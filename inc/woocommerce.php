...
add_action( 'after_setup_theme', 'wpcore_woocommerce_support' );
if ( ! function_exists( 'wpcore_woocommerce_support' ) ) {
	/**
	 * Declares WooCommerce theme support.
	 */
	function wpcore_woocommerce_support() {
		...
		add_filter( 'woocommerce_form_field_args', 'wpcore_wc_form_field_args', 10, 3 );
		add_filter( 'woocommerce_form_field_radio', 'wpcore_wc_form_field_radio', 10, 4 );
		add_filter( 'woocommerce_quantity_input_classes', 'wpcore_quantity_input_classes' );
		add_filter( 'woocommerce_loop_add_to_cart_args', 'wpcore_loop_add_to_cart_args' );

		// Wrap the add-to-cart link in `div.add-to-cart-container`.
		add_filter( 'woocommerce_loop_add_to_cart_link', 'wpcore_loop_add_to_cart_link' );

		// Add Bootstrap classes to account navigation.
		add_filter( 'woocommerce_account_menu_item_classes', 'wpcore_account_menu_item_classes' );
	}
}
...
add_action( 'woocommerce_before_main_content', 'wpcore_woocommerce_wrapper_start', 10 );
add_action( 'woocommerce_after_main_content', 'wpcore_woocommerce_wrapper_end', 10 );

if ( ! function_exists( 'wpcore_woocommerce_wrapper_start' ) ) {
	/**
	 * Display the theme specific start of the page wrapper.
	 */
	function wpcore_woocommerce_wrapper_start() {
		$container = get_theme_mod( 'wpcore_container_type' );
		if ( false === $container ) {
			$container = '';
		}
...
if ( ! function_exists( 'wpcore_woocommerce_wrapper_end' ) ) {
	/**
	 * Display the theme specific end of the page wrapper.
	 */
	function wpcore_woocommerce_wrapper_end() {
		echo '</main>';
...
if ( ! function_exists( 'wpcore_wc_form_field_args' ) ) {
	/**
	 ...
	 */
	function wpcore_wc_form_field_args( $args, $key, $value ) {
		$bootstrap4 = 'bootstrap4' === get_theme_mod( 'wpcore_bootstrap_version', 'bootstrap4' );
...
if ( ! function_exists( 'wpcore_wc_form_field_radio' ) ) {
	/**
	 ...
	 * `wpcore_wc_form_field_args()` that is meant for input labels only.
	 *
	 ...
	 * @phpstan-param T $args
	 */
	function wpcore_wc_form_field_radio( $field, $key, $args, $value ) {
...
if ( ! function_exists( 'wpcore_quantity_input_classes' ) ) {
	/**
	 ...
	 */
	function wpcore_quantity_input_classes( $classes ) {
		$classes[] = 'form-control';
		return $classes;
	}
}

if ( ! function_exists( 'wpcore_loop_add_to_cart_link' ) ) {
	/**
	 ...
	 */
	function wpcore_loop_add_to_cart_link( $html ) {
		return '<div class="add-to-cart-container">' . $html . '</div>';
	}
}

if ( ! function_exists( 'wpcore_loop_add_to_cart_args' ) ) {
	/**
	 ...
	 */
	function wpcore_loop_add_to_cart_args( $args ) {
...
if ( ! function_exists( 'wpcore_account_menu_item_classes' ) ) {
	/**
	 ...
	 */
	function wpcore_account_menu_item_classes( $classes ) {
