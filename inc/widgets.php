...
if ( ! function_exists( 'wpcore_add_widget_categories_class' ) ) {
	/**
	 ...
	 */
	function wpcore_add_widget_categories_class( $cat_args ) {

		if ( isset( $cat_args['class'] ) ) {
			$cat_args['class'] .= ' ' . wpcore_get_select_control_class();
		} else {
			$cat_args['class'] = wpcore_get_select_control_class();
		}
...
add_filter( 'widget_categories_dropdown_args', 'wpcore_add_widget_categories_class' );

if ( ! function_exists( 'wpcore_add_block_widget_categories_class' ) ) {
	/**
	 ...
	 */
	function wpcore_add_block_widget_categories_class( $output, $parsed_args ) {
		$class = wpcore_get_select_control_class();
...
add_filter( 'wp_dropdown_cats', 'wpcore_add_block_widget_categories_class', 10, 2 );

if ( ! function_exists( 'wpcore_add_block_widget_archives_classes' ) ) {
	/**
	 ...
	 */
	function wpcore_add_block_widget_archives_classes( $block_content, $block ) {

		if ( isset( $block['attrs']['displayAsDropdown'] ) && true === $block['attrs']['displayAsDropdown'] ) {
			return str_replace(
				'<select',
				'<select class="' . wpcore_get_select_control_class() . '"',
				$block_content
			);
		}
...
add_filter( 'render_block_core/archives', 'wpcore_add_block_widget_archives_classes', 10, 2 );

if ( ! function_exists( 'wpcore_add_block_widget_search_classes' ) ) {
	/**
	 ...
	 * @return string The filtered block content.
	 */
	function wpcore_add_block_widget_search_classes( $block_content, $block ) {
...
add_filter( 'render_block_core/search', 'wpcore_add_block_widget_search_classes', 10, 2 );
 ...
 */
function wpcore_hero_active_carousel_item( $params ) {
	if (
		! isset( $params[0] )
		|| ! isset( $params[0]['id'] )
		|| 'hero' !== $params[0]['id']
...
add_filter( 'dynamic_sidebar_params', 'wpcore_hero_active_carousel_item' );
 ...
 */
add_filter( 'dynamic_sidebar_params', 'wpcore_widget_classes' );

if ( ! function_exists( 'wpcore_widget_classes' ) ) {
	 ...
	 * @return array Maybe filtered parameters.
	 */
	function wpcore_widget_classes( $params ) {
...
} // End of if function_exists( 'wpcore_widget_classes' ).
	...
	function wpcore_widgets_init() {
		register_sidebar(
			array(
				...
				'name'          => __( 'Right Sidebar', 'wpcore' ),
				'id'            => 'right-sidebar',
				'description'   => __( 'Right sidebar widget area', 'wpcore' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Left Sidebar', 'wpcore' ),
				'id'            => 'left-sidebar',
				'description'   => __( 'Left sidebar widget area', 'wpcore' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h3 class="widget-title">',
				'after_title'   => '</h3>',
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Hero Slider', 'wpcore' ),
				'id'            => 'hero',
				'description'   => __( 'Hero slider area. Place two or more widgets here and they will slide!', 'wpcore' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Hero Canvas', 'wpcore' ),
				'id'            => 'herocanvas',
				'description'   => __( 'Full size canvas hero area for Bootstrap and other custom HTML markup', 'wpcore' ),
				'before_widget' => '',
				'after_widget'  => '',
				'before_title'  => '',
				'after_title'   => '',
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Top Full', 'wpcore' ),
				'id'            => 'statichero',
				'description'   => __( 'Full top widget with dynamic grid', 'wpcore' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Footer Full', 'wpcore' ),
				'id'            => 'footerfull',
				'description'   => __( 'Full sized footer widget with dynamic grid', 'wpcore' ),
...
} // End of function_exists( 'wpcore_widgets_init' ).
